...
void drawObjectPBR(GLuint program, Core::RenderContext& context, glm::mat4 modelMatrix, glm::vec3 color, float roughness, float metallic) {
	...
	glm::vec3 sunPosition = glm::eulerAngleY(timeGL / 3) * glm::vec4(1.f, 0.0f, 0.f, 1.f);
	sunDir = glm::normalize(-sunPosition);
	...
	Core::SetActiveTexture(texture::clouds, "texture2", program, 1);
	Core::SetActiveTexture(texture::earthNormal, "textureNormal", program, 2);
